...
if($listcount>1){
	 ...
	 if($huoduan['link_open']==0){
	  ...
	  if(is_file(ROOT_PATH.'/data/huoduan.killurls.txt')){
	   ...
	   if($kurl!=1){
	  ?>
  <div class="g"><h2><?php if($huoduan['listnum']==1){?><span class="nums"><?php echo $listnum[$ii]?></span> <?php }?><a href="<?php echo $sourl?>" target="_blank" class="s" rel="nofollow"><?php echo $list['data'][$i]['title']?></a></h2><div class="std"><?php echo $list['data'][$i]['des']?></div><span class="a"><?php echo $blink?></span></div>
	  <?php	
       }
	}
