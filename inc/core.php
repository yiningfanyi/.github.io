...
if(isset($huoduan['searchsiteopen']) && $huoduan['searchsiteopen']==1){
...
function huoduan_get_baidu($q,$p=1,$time=86400){
	...
	if(is_file($file) && $xtime){
	    ...
	    $html = huoduan_get_html('http://www.baidu.com/s?wd='.SEARCHSITE.$s.'&pn='.(($p-1)*10).'&ie=utf-8','http://www.baidu.com/','6');
...
function huoduan_get_newbaidu($q,$p=1,$time=86400){
	...
	if(is_file($file) && $xtime){
	    ...
	    $html = huoduan_get_html('http://www.baidu.com/s?wd='.SEARCHSITE.$s.'&pn='.(($p-1)*10).'&tn=baidulaonian&ie=utf-8','http://www.baidu.com/','6');
			$body = huoduan_get_body($html,'<ol>','</ol>',1);$s=substr(e(1),-2,1);
...
function huoduan_get_haosou_xg($q,$time=1){
	...
	if(is_file($file)){
       ...
       $html = huoduan_get_html('http://www.so.com/s?ie=utf-8&shb=1&src=360sou_newhome&q='.$s,'http://www.so.com/','6');
		if(strpos($html,'<div id="rs">')){
			$body = huoduan_get_body($html,'<div id="rs">','</div>',1);
