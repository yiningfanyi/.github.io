...
<div class="searchbox">
 <form action="./" method="get" onsubmit="return subck();"><input align="middle" name="q" class="q" id="kw" value="请输入关键字搜索网页" onfocus="javascript:if(this.value=='请输入关键字搜索网页'){this.value='';this.style.color='#333';this.style.borderColor='#FC8105';}" onblur="javascript:if(this.value==''){this.value='请输入关键字搜索网页';this.style.color='#CCC';this.style.borderColor='#CFC7C8';}" maxlength="100" size="50" autocomplete="off" baiduSug="1" /><?php if(REWRITE=='1'){?><input name="re" type="hidden" value="1" /><?php }?><input id="btn" class="btn" align="middle" value="搜索一下" type="submit" />
        </form>
</div>
<?php
...
<div class="hotsearch">
  ...
  <ul id="toplist1">
     <?php 
	     //以下代码后面的“1” 就是百度热搜榜的ID，比如 http://top.baidu.com/buzz?b=26&c=1&fr=topcategory_c1 这个栏目的ID就是26
...
<?php
if(is_file('data/huoduan.links.txt')){
  $links = file_get_contents('data/huoduan.links.txt');
}else{
  $links = file_get_contents('../data/huoduan.links.txt');
}
$links = trim($links,"\r\n");
