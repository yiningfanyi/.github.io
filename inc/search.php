...
<div id="header">
  <div class="con">
      ...
      <div class="searchbox">
       <form action="<?php echo SYSPATH?>" method="get" onsubmit="return subck();"><input align="middle" name="q" class="q" id="kw" value="<?php echo $q?>" maxlength="100" size="50" autocomplete="off" baiduSug="1" /><?php if(REWRITE=='1'){?><input name="re" type="hidden" value="1" /><?php }?><input id="btn" class="btn" align="middle" value="搜索一下" type="submit" />
              </form>
      </div>
...
if($listcount>1&& $kill!=1){
  ...
  for($i=0;$i<$listcount;$i++){
	  ...
	  if($kurl!=1 && strlen($list['data'][$ni]['title'])>0 ){
		...
		?>
	<div class="g"><h2><?php if($huoduan['listnum']==1){?><span class="nums"><?php echo $listnum[$ii]?></span> <?php }?><a href="<?php echo $sourl?>" target="_blank" class="s" rel="nofollow"><?php echo $list['data'][$ni]['title']?></a></h2><div class="std"><?php echo $list['data'][$ni]['des']?></div><span class="a"><?php echo huoduan_msubstr($blink,0,60);?></span></div>
		<?php	
	  }
  }
  echo '</div>';
